...
def init_maps():
    # map each block to a mapper function
    maps = {}
...
if __name__ == "__main__":
    ...
    seeds = deque(maps["seeds"])
    minlocation: int = 1 << 32
    while len(seeds) > 0:
        s1 = seeds.popleft()
        s2 = seeds.popleft()
        for idx, seed in enumerate(range(s1, s1 + s2)):
            location = get_location(maps, seed)
            minlocation = min(minlocation, location)
            print(f"seeds left: {len(seeds)} - progress with this seed: {(idx / (s1 + s2 - 1) * 100):.2f}%")
    print("solution, part 2:", minlocation)
