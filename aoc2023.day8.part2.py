...
curr = [l[0] for l in lines if l[0][2] == "A"]
cmds = fstline
steps = 0

lookup = lambda x: list(filter(lambda l: l[0] == x, lines))[0][1]
while len(cmds) > 0:
    # execute cmd for each key
    if cmds[0] == "R":
        curr = [lookup(c)[1] for c in curr]
    else:
        curr = [lookup(c)[0] for c in curr]

    # update cmds
    cmds = cmds[1:]
    reachedgoal = all(map(lambda x: x[2] == "Z", curr))
    if len(cmds) == 0 and not reachedgoal:
        cmds = fstline

    # count steps
    steps += 1
    print(curr, " - cmds:", cmds, " - steps:", steps)
